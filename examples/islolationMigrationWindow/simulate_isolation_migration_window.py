...
def migStopDemography():
	...
	demographic_events = [
		# start migration
        msprime.MigrationRateChange(time=T_stop, rate=m, matrix_index=(0, 1)),
        msprime.MigrationRateChange(time=T_stop, rate=m, matrix_index=(1, 0)),
		# merge both populations
		msprime.MassMigration(
			time=T_div, source=1, destination=0, proportion=1.0),
		msprime.PopulationParametersChange(
			time=T_div, initial_size=N_A, growth_rate=0, population_id=0),
		# stop migration
        msprime.MigrationRateChange(time=T_div, rate=0, matrix_index=(0, 1)),
        msprime.MigrationRateChange(time=T_div, rate=0, matrix_index=(1, 0))
	]
