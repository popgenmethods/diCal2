...
public abstract class GeneralGeneticTypeConfig<H extends GeneticType> implements GeneticTypeConfig<H> {
	// the configuration
	private final Map<H, GeneticType.GeneticTypeMultiplicity<H>> typeConfiguration;
	
	// cached information about the configuration
	private int numGeneticTypes;
	private int distinctGeneticTypes;

	public GeneralGeneticTypeConfig() {
		this.typeConfiguration = new LinkedHashMap<H, GeneticType.GeneticTypeMultiplicity<H>>();
	...
	public void adjustType(H type, int adjustment) {
		GeneticType.GeneticTypeMultiplicity<H> geneMultiplicity = typeConfiguration.get(type);
		
		if (geneMultiplicity == null) {
			if (adjustment != 0) {
				typeConfiguration.put(type, new GeneticType.GeneticTypeMultiplicity<H>(type, adjustment));
		...
		updateCachedItems(type, adjustment);
	}

	public abstract void updateCachedItems(H type, int adjustment);

	public final void addTypes(Iterable<GeneticType.GeneticTypeMultiplicity<H>> configuration) {
		for (GeneticType.GeneticTypeMultiplicity<H> typeMultiplicity : configuration) {
			adjustType(typeMultiplicity.geneticType, typeMultiplicity.multiplicity);
		}
	}

	public final int getMultiplicity(H type) {
		GeneticType.GeneticTypeMultiplicity<H> typeMultiplicity = typeConfiguration.get(type);
		return typeMultiplicity == null ? 0 : typeMultiplicity.multiplicity;
	}
	
	public final Iterator<GeneticType.GeneticTypeMultiplicity<H>> iterator() {
	...
	public String toString() {
		...
		for (GeneticType.GeneticTypeMultiplicity<H> typeMultiplicity : this) {
